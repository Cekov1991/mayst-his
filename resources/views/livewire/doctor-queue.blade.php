<div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <div class="overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 py-5 shadow sm:p-6">
            <dt class="truncate text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('doctor_queue.queue') }}</dt>
            <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ $stats['total'] }}</dd>
        </div>

        <div class="overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 py-5 shadow sm:p-6">
            <dt class="truncate text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('doctor_queue.arrived') }}</dt>
            <dd class="mt-1 text-3xl font-semibold tracking-tight text-yellow-600 dark:text-yellow-400">{{ $stats['arrived'] }}</dd>
        </div>

        <div class="overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 py-5 shadow sm:p-6">
            <dt class="truncate text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('doctor_queue.in_progress') }}</dt>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
        <div class="flex items-center space-x-4">
            <div>
                <label for="selectedDate" class="block text-sm font-medium text-gray-900 dark:text-white">{{__('common.table_header.date')}}</label>
                <input type="date"
                       id="selectedDate"
        ...
        <div class="text-sm text-gray-500 dark:text-gray-400">
            ...
            <span wire:loading.remove>
                {{__('common.last_updated')}}: {{ now()->format('g:i A') }}
            </span>
        </div>
    </div>
                        ...
                        <div class="flex space-x-2">
                            ...
                            <a href="{{ route('visits.show', $visit) }}"
                               class="text-sm font-medium text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300">
                                {{__('common.table_cell.details')}}<span class="sr-only">, {{ $visit->patient->full_name }}</span>
                            </a>
                        </div>
                    </x-table-action-cell>
                </x-table-row>
