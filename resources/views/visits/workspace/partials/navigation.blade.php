    ...
    <div class="border-b border-gray-200 dark:border-gray-700">
        <nav class="-mb-px flex space-x-8 px-6" aria-label="Tabs">
            ...
            <a href="{{ route('visits.diagnoses', $visit) }}"
               class="@if($active === 'diagnoses') border-indigo-500 text-indigo-600 dark:text-indigo-400 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                {{ __('workspace.diagnoses') }}
    ...
    <div class="px-6 py-3 bg-gray-50 dark:bg-gray-900/20 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between text-sm">
            <div class="flex items-center space-x-4 text-gray-600 dark:text-gray-400">
                <span><strong>{{ __('visits.patient') }}:</strong> {{ $visit->patient->full_name }}</span>
                <span><strong>{{ __('visits.type') }}:</strong> {{ __("visits.types.{$visit->type}") }}</span>
                <span><strong>{{ __('visits.status') }}:</strong>
