<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('his.diagnoses.add_diagnosis') }} - {{ $visit->patient->full_name }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('his.diagnoses.add_diagnosis') }}</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="eye" id="eye"
                                    ...
                                    <option value="OD" {{ old('eye') === 'OD' ? 'selected' : '' }}>{{ __('his.diagnoses.od') }}</option>
                                    <option value="OS" {{ old('eye') === 'OS' ? 'selected' : '' }}>{{ __('his.diagnoses.os') }}</option>
                                    <option value="OU" {{ old('eye') === 'OU' ? 'selected' : '' }}>{{ __('his.diagnoses.ou') }}</option>
                                    <option value="NA" {{ old('eye') === 'NA' ? 'selected' : '' }}>{{ __('his.diagnoses.na') }}</option>
                                </select>
                                @error('eye')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                        class="mt-1 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 dark:bg-white/5 dark:text-white dark:ring-white/10 dark:focus:ring-indigo-500" required>
                                    <option value="provisional" {{ old('status', 'provisional') === 'provisional' ? 'selected' : '' }}>{{ __('his.diagnoses.provisional') }}</option>
                                    <option value="working" {{ old('status') === 'working' ? 'selected' : '' }}>{{ __('his.diagnoses.working') }}</option>
                                    <option value="confirmed" {{ old('status') === 'confirmed' ? 'selected' : '' }}>{{ __('his.diagnoses.confirmed') }}</option>
                                    <option value="ruled_out" {{ old('status') === 'ruled_out' ? 'selected' : '' }}>{{ __('his.diagnoses.ruled_out') }}</option>
                                    <option value="resolved" {{ old('status') === 'resolved' ? 'selected' : '' }}>{{ __('his.diagnoses.resolved') }}</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="severity" id="severity"
                                        class="mt-1 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 dark:bg-white/5 dark:text-white dark:ring-white/10 dark:focus:ring-indigo-500" required>
                                    <option value="unknown" {{ old('severity', 'unknown') === 'unknown' ? 'selected' : '' }}>{{ __('his.diagnoses.unknown') }}</option>
                                    <option value="mild" {{ old('severity') === 'mild' ? 'selected' : '' }}>{{ __('his.diagnoses.mild') }}</option>
                                    <option value="moderate" {{ old('severity') === 'moderate' ? 'selected' : '' }}>{{ __('his.diagnoses.moderate') }}</option>
                                    <option value="severe" {{ old('severity') === 'severe' ? 'selected' : '' }}>{{ __('his.diagnoses.severe') }}</option>
                                </select>
                                @error('severity')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="acuity" id="acuity"
                                        class="mt-1 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 dark:bg-white/5 dark:text-white dark:ring-white/10 dark:focus:ring-indigo-500" required>
                                    <option value="unknown" {{ old('acuity', 'unknown') === 'unknown' ? 'selected' : '' }}>{{ __('his.diagnoses.unknown') }}</option>
                                    <option value="acute" {{ old('acuity') === 'acute' ? 'selected' : '' }}>{{ __('his.diagnoses.acute') }}</option>
                                    <option value="subacute" {{ old('acuity') === 'subacute' ? 'selected' : '' }}>{{ __('his.diagnoses.subacute') }}</option>
                                    <option value="chronic" {{ old('acuity') === 'chronic' ? 'selected' : '' }}>{{ __('his.diagnoses.chronic') }}</option>
                                </select>
                                @error('acuity')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="code_system" id="code_system"
                                    ...
                                    <option value="LOCAL" {{ old('code_system') === 'LOCAL' ? 'selected' : '' }}>{{ __('his.diagnoses.local') }}</option>
                                </select>
                                @error('code_system')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="notes" id="notes" rows="4"
                                          class="mt-1 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 dark:bg-white/5 dark:text-white dark:ring-white/10 dark:placeholder:text-gray-500 dark:focus:ring-indigo-500"
                                          placeholder="{{ __('his.diagnoses.notes_placeholder') }}">{{ old('notes') }}</textarea>
                                @error('notes')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
